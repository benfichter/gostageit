...
Standalone version - no external dependencies except MoGe
"""
...
def run_placement_pipeline(
    ...
    skip_detection: bool = True  # Always skip detection in standalone version
) -> Dict:
    """
    ...
    Args:
        ...
        style_hints: Optional style description (future feature)
        model: Optional pre-loaded MoGe model
        device: Device for computation
        use_3d_rendering: Whether to use full 3D rendering
        skip_detection: Skip existing furniture detection (always True in standalone)
    ...
    # Step 2: Skip furniture detection (assume empty room)
    log("Assuming empty room (standalone version)")
    existing_furniture = []
...
def create_dimension_overlay(image_rgb: np.ndarray, placements: List[Dict],
    ...
    for i, placement in enumerate(placements):
        y_offset += 25
        if y_offset > h - 20:
            break  # Prevent text from going off screen
