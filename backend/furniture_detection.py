...
class FurnitureDetector:
    """
    Segments volumetric furniture clusters directly from MoGe depth/normal outputs.
    """
    ...
    def __init__(
        ...
        min_point_count: int = 150,
        smoothing_kernel: int = 5,
        dilation_iterations: int = 1,
    ) -> None:
        self.log = log_fn or default_log
        self.min_area_pixels = max(1, min_area_pixels)
        self.min_point_count = max(1, min_point_count)
    ...
    def detect(
        ...
        # Detection is geometry-driven, so the RGB frame is unused aside from keeping
        # the previous call signature consistent for callers.
        _ = image_rgb
